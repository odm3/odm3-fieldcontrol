...
// autonomous/driver (DESIGN §1) — so a Driver consumes exactly a match.Output and
// is opaque about how it reaches the robot. A registry lets the run loop select a
// driver (mock for tests, V5 GPIO on real hardware) by name (DESIGN §13 Phase 1).
package driver
...
type Driver interface {
	Name() string
	Apply(match.Output) error
	Close() error
}

// Factory constructs a Driver. Registration uses factories so opening the hardware
// (e.g. GPIO lines) is deferred until a field actually selects the driver.
